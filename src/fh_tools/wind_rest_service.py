...
ERROR_CODE_MSG_DIC = {
    ...
    -40521010: "网络超时",
    -40522017: "数据提取量超限",
    -40520004: "登陆失败。请结束相关Wind进程（wbox）并重启终端",
}
    ...
    def post(self):
        ...
        for n_data in range(data_len):
            ...
            if data_len2 > 0:
                ...
                if item_check is not None and type(item_check) in (datetime, date):
                    ret_data.Data[n_data] = [format_2_date_str(dt) for dt in data]
                    logger.debug('%d column["%s"]  date to str', n_data, ret_data.Fields[n_data])
        # 组成 DataFrame
...
def set_log_level(sleep_time=5):
    logger.info('invoked %d', sleep_time)
    time.sleep(sleep_time)
    logging.getLogger('werkzeug').setLevel(logging.WARNING)
    logger.info('set log werkzeug level to WARNING')


def start_service():
    if w.isconnected():
        w.start()
        set_level_thread = Thread(target=set_log_level, args=(6,))
        set_level_thread.start()
