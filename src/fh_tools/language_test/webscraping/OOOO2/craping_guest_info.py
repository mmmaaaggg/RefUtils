...
@try_n_times(5, exception_sleep_time=5)
    ...
    try:
        if fileUrl:
            ...
            with open(filename, 'wb') as outfile:
                urlfile = urlopener.open(urllib.request.Request(fileUrl), timeout=URL_OPEN_TIMEOUT)
...
@try_n_times(3, exception_sleep_time=3)
def login_website(urlopener, username, password, paramstr):
    ...
    urlcontent = urlopener.open(
        urllib.request.Request(url, urllib.parse.urlencode(values).encode(encoding='UTF8')),
