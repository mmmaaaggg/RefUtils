...
import urllib.request, urllib.error, urllib.parse, os, os.path, urllib.request, urllib.parse, urllib.error, random
...
def main(page):
    ...
    print('@@@@@@@@@@@@@@@@第 %d 页共有 %d 个主题@@@@@@@@@@@@@@@@' %(page,len(menu)))
    for i in menu:
...
if __name__=='__main__':
    ...
    pages=get_pages(url)
    print('***************妹子图一共有 %d 页******************' %pages)
    if not os.path.exists('MeiZiTu'):
        os.mkdir('MeiZiTu')
    page_start=eval(input('Input the first page number:\n'))
    page_end=eval(input('Input the last page number:\n'))
