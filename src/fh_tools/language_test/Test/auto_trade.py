...
class OperationThs:
    def __init__(self):
        ...
        try:
            top_hwnd = self._find_trade_client_hwnd()  # 交易窗口
            self.__app = pywinauto.application.Application()
            self.__app.connect(handle=top_hwnd)
            # self.__app.connect(title='网上股票交易系统5.0')
            # top_hwnd = pywinauto.findwindows.find_window(title='网上股票交易系统5.0')
            dialog_hwnd = \
            pywinauto.findwindows.find_windows(top_level_only=False, class_name='#32770', parent=top_hwnd)[1]
            ...
            if len(wanted_hwnds) not in (99, 97, 96, 98, 100, 101, 106):
                tkinter.messagebox.showerror('错误', '无法获得“同花顺双向委托界面”的窗口句柄,请将同花顺交易系统切换到“双向委托界面”！')
                exit()
            self._main_window = self.__app.window_(handle=top_hwnd)
            self._dialog_window = self.__app.window_(handle=dialog_hwnd)
        except:
            pass
    ...
    def _find_trade_client_hwnd(self):
        ...
        if self.lpClassName is None:
            ...
            win32gui.EnumWindows(lambda hWnd, param:
                                 OperationThs._filter_trade_client(r'Afx:400000:0:0:.+:0', hWnd, param),
                                 hwnd_list)
            if len(hwnd_list) > 0:
    ...
    def __buy(self, code, quantity):
        ...
        """
        self._dialog_window.Edit1.SetFocus()
        time.sleep(0.2)
        self._dialog_window.Edit1.SetEditText(code)
        time.sleep(0.2)
        if quantity != '0':
            self._dialog_window.Edit3.SetEditText(quantity)
            time.sleep(0.2)
        self._dialog_window.Button1.Click()
        time.sleep(0.2)
    ...
    def __sell(self, code, quantity):
        ...
        :param quantity: 数量， 字符串
        """
        self._dialog_window.Edit4.SetFocus()
        time.sleep(0.2)
        self._dialog_window.Edit4.SetEditText(code)
        time.sleep(0.2)
        if quantity != '0':
            self._dialog_window.Edit6.SetEditText(quantity)
            time.sleep(0.2)
        self._dialog_window.Button2.Click()
        time.sleep(0.2)
    ...
    def __closePopupWindow(self):
        ...
        """
        popup_hwnd = self._main_window.PopupWindow()
        if popup_hwnd:
    ...
    def maxWindow(self):
        ...
        最大化窗口
        """
        if self._main_window.GetShowState() != 3:
    ...
    def minWindow(self):
        ...
        最小化窗体
        """
        if self._main_window.GetShowState() != 2:
    ...
    def refresh(self, t=0.5):
        ...
        :param t:刷新后的等待时间
        """
        self._dialog_window.Button5.Click()
        time.sleep(t)
    ...
    def getMoney(self):
        ...
        获取可用资金
        """
        return float(self._dialog_window.Static19.WindowText())
    ...
    def __copyToClipboard(self):
        ...
        :return:
        """
        self._dialog_window.CVirtualGridCtrl.RightClick(coords=(30, 30))
        self._main_window.TypeKeys('C')
    ...
    def __selectWindow(self, choice):
        ...
        :return:
        """
        rect = self._dialog_window.CCustomTabCtrl.ClientRect()
        x = rect.width() // 8
        y = rect.height() // 2
        if choice == 'W':
            x = x
            ...
            x *= 7
        self._dialog_window.CCustomTabCtrl.ClickInput(coords=(x, y))
        time.sleep(0.5)
    ...
    def withdraw(self, code, direction):
        ...
        if row_pos:
            for row in row_pos:
                self._dialog_window.CVirtualGridCtrl.ClickInput(coords=(7, 28 + 16 * row))
            self._dialog_window.Button12.Click()
            self.__closePopupWindows()
    ...
    def withdrawBuy(self):
        ...
        self._dialog_window.Button8.Click()
        self.__closePopupWindows()
    ...
    def withdrawSell(self):
        ...
        """
        self.__selectWindow(choice='R')
        self._dialog_window.Button9.Click()
        self.__closePopupWindows()
    ...
    def withdrawAll(self):
        ...
        """
        self.__selectWindow(choice='R')
        self._dialog_window.Button7.Click()
        self.__closePopupWindows()
...
class StockGui:
    ...
    def about(self):
        tkinter.messagebox.showinfo("关于",
                                    '\r此系统仅适应于同花顺网上交易5.0，使用时请先登陆同花顺网上交易系统并切换到“同花顺双向委托界面”。\r 版本号：v 1.0.0 \r 作者：水域\r 发布日期：2017.01.11')
